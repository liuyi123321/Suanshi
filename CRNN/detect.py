...
def detect(model, image_path, device, converter):
    ...
    # 对图片进行resize和归一化操作
    transform = ResizeAndNormalize((266, 32))
...
def main(opt):
    ...
    dict_temp = {'image_path': image_path_list, 'detection': answer_list}
    result = pd.DataFrame(data=dict_temp)
    result.to_csv(f'{os.getcwd()}/CRNN/detect_result/result.csv')
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str,
                        default='../yolov5-master/runs/detect/exp13/crops/equation/',
                        help='要用来推理图片的路径，可以是一张图片，也可以是一个目录')
    parser.add_argument('--batch_size', type=int, default=32, help='批次大小')
    parser.add_argument('--chinese', type=str, default=f'{Root}/CRNN/data/chinese.txt', help='字符集保存路径')
